...
class TestEncoders(unittest.TestCase):
    ...
    def test_leave_one_out(self):
        ...
        self.verify_numeric(enc.transform(X_t, y_t, random_mized=True, sigma=0.1))
        self.verify_numeric(enc.transform(X_t, y_t, random_mized=True, sigma=np.random.rand(X_t.shape[0])))
        self.verify_numeric(enc.transform(X_t, random_mized=True))
        self.verify_numeric(enc.transform(X_t, random_mized=True, sigma=0.1))
        self.verify_numeric(enc.transform(X_t, random_mized=True, sigma=np.random.rand(X_t.shape[0])))
        ...
        self.assertTrue(isinstance(enc.transform(X_t, random_mized=True), np.ndarray))
        self.assertTrue(isinstance(enc.transform(X_t, y_t, random_mized=True, sigma=0.1), np.ndarray))
        self.assertTrue(
            isinstance(enc.transform(X_t, random_mized=True, sigma=np.random.rand(X_t.shape[0])), np.ndarray))
        self.assertTrue(
            isinstance(enc.transform(X_t, y_t, random_mized=True, sigma=np.random.rand(X_t.shape[0])), np.ndarray))
