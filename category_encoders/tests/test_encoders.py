...
class TestEncoders(TestCase):
    ...
    def test_impact_encoders(self):
        for encoder_name in ['LeaveOneOutEncoder', 'TargetEncoder', 'WOEEncoder', 'MEstimateEncoder', 'JamesSteinEncoder', 'CatBoostEncoder', 'MixedEncoder']:
            with self.subTest(encoder_name=encoder_name):
    ...
    def test_get_feature_names(self):
        for encoder_name in encoders.__all__:
            with self.subTest(encoder_name=encoder_name):
                ...
                if encoder_name not in ['TargetEncoder', 'WOEEncoder', 'LeaveOneOutEncoder', 'MEstimateEncoder', 'JamesSteinEncoder', 'CatBoostEncoder', 'MixedEncoder']:
    ...
    def test_get_feature_names_drop_invariant(self):
        ...
        for encoder_name in encoders.__all__:
            with self.subTest(encoder_name=encoder_name):
                enc = getattr(encoders, encoder_name)(drop_invariant=True)
                # Target encoders also need y
                if encoder_name not in ['TargetEncoder', 'WOEEncoder', 'LeaveOneOutEncoder', 'MEstimateEncoder', 'JamesSteinEncoder', 'CatBoostEncoder', 'KFoldEncoder', 'MixedEncoder']:
                    obtained = enc.fit(X).get_feature_names()
                    expected = enc.transform(X).columns.tolist()
                else:
                    obtained = enc.fit(X, y).get_feature_names()
    ...
    def test_target_encoders(self):
        # See issue #206
        for encoder_name in {'CatBoostEncoder', 'JamesSteinEncoder', 'LeaveOneOutEncoder', 'MEstimateEncoder', 'TargetEncoder', 'WOEEncoder', 'MixedEncoder'}:
            with self.subTest(encoder_name=encoder_name):
                enc = getattr(encoders, encoder_name)(return_df=False)
