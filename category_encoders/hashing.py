...
class NHashingEncoder(HashingEncoder):
    ...
    def __init__(self, verbose=0, n_components=8, cols=None,
                 drop_invariant=False, return_df=True, hash_method='md5',
                 max_process=1, max_sample=0):
        """

        :param verbose: integer indicating verbosity of the output. 0 for none.
        :param n_components:
        :param cols: a list of columns to encode, if None, all string columns will be encoded.
        :param drop_invariant: boolean for whether or not to drop columns with 0 variance.
        :param return_df: boolean for whether to return a pandas DataFrame from transform (otherwise it will be a numpy array).
        :param hash_method: which hashing method to use. Any method from hashlib works.
        :param max_process: how many processes to do hashing at same time
        :param max_sample: how many samples each process get from dataset at each time
        """

        super(NHashingEncoder, self).__init__(
            verbose=verbose, n_components=n_components, cols=cols,
            drop_invariant=drop_invariant, return_df=return_df,
            hash_method=hash_method)

        self.max_process = max_process if max_process in range(1, 64) else 1
    ...
    def __hash_transform(self, X_part, override_return_df=False):
        return super(NHashingEncoder, self).transform(X=X_part,
    ...
    def __require_data(self, cols, process_index, override_return_df):
        if self.data_lock.acquire():
            ...
            if all([self.data_lines > 0, done_index < self.data_lines]):
                ...
                self.hashing_parts.put(self.__hash_transform(X_part=data_part,
                                                             override_return_df=override_return_df))
    ...
    def n_fit(self, X, y=None, **kwargs):
        return super(NHashingEncoder, self).fit(X=X, y=y, kwargs=kwargs)

    def n_transform(self, X, override_return_df=False):
        ...
        if self.max_sample == 0 or self.max_sample == self.data_lines:
            ...
            data = pd.concat(concat_data, ignore_index=True).reset_index(
                drop=True)
            return data
